...
public class KafkaConfig {
    ...
    @Value("${processor.bean.event.producer.id}")
    private String beanEventProducerId;

    @Bean
    public KafkaProducer<String, BeanSupplied> kafkaBeanEventProducer(Properties kafkaProperties) {
        Properties props = (Properties) kafkaProperties.clone();
        props.setProperty(CLIENT_ID_CONFIG, beanEventProducerId);
